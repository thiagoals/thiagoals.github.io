...
export class AplicacoesComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    await this.appliancesClient.execute('CREDIT',this.page-1,this.item).subscribe({
      next: (r: any) => {
  ...
  reloadPage(page:number){
    if(page<=this.totalPages && page>=1)
      this.router.navigate([`/admin/aplicacoes/${page}/${this.item}`]).then(page => { window.location.reload(); });
  }
}
